...
function vowelCount(str) {
    const vowels = "aeiou";
    str = str.toLowerCase();
    return str.split('').reduce(function (accumulator, curValue) {
        if (vowels.includes(curValue)) {
            if (accumulator[curValue]) {
                accumulator[curValue] ++;
            } else (accumulator[curValue] = 1)
        }
        return accumulator;
    }, {});
}
...
function addKeyAndValue(arr, key, value) {
    return arr.reduce(function (accumulator, curValue, index) {
